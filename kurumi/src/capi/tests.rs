...
use crate::capi::eval::*;
use crate::capi::graph::leaves::*;
use crate::capi::graph::serialize::*;
use crate::capi::graph::*;
use crate::capi::ops::nn::{activation::*, conv::*, pool::*};
use crate::capi::ops::*;
use crate::capi::ops::{contract::*, indexing::*, linalg::*, movement::*, random::*, signal::*, spatial::*};
...
// ku_pad_mode must reproduce the Rust `Graph::pad_mode` builder (reflect on a small tensor),
// and reject an unknown mode.
#[test]
fn capi_pad_mode_matches_builder() {
    unsafe {
        let data = [1f32, 2., 3., 4.];
        // C ABI: reflect-pad by (1,1), mode 0 = reflect.
        let g = ku_graph_new();
        let cpu = ku_backend_new(0);
        let x = ku_constant_f32(g, data.as_ptr(), 4, [4usize].as_ptr(), 1);
        let padded = ku_pad_mode(g, x, [1usize, 1].as_ptr(), 1, 0);
        assert_ne!(padded, KU_ERR);
        let got = read_f32(ku_eval(g, padded, cpu));

        // Rust builder oracle for the same op.
        let mut gr = kurumi_core::Graph::new();
        let rx = gr.const_storage(kurumi_core::Storage::F32(data.to_vec()), vec![4]);
        let rp = gr.pad_mode(rx, vec![(1, 1)], "reflect").unwrap();
        let want = match kurumi_core::interpret(&gr, rp).storage {
            kurumi_core::Storage::F32(v) => v,
            _ => unreachable!(),
        };
        assert_eq!(got, want, "ku_pad_mode diverged from Graph::pad_mode");

        // unknown mode -> KU_ERR + message
        assert_eq!(ku_pad_mode(g, x, [1usize, 1].as_ptr(), 1, 99), KU_ERR);
        assert!(!ku_last_error().is_null());
