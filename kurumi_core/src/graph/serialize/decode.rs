...
//! (shape/dtype re-inferred) then read the entry table, plus the byte Reader. The exhaustive
//! per-Op decode match (read_op) lives in `op` and is re-exported here.

mod op;

pub(super) use op::read_op;

use crate::graph::serialize::{Entry, InputBinding, InputRole, MAGIC, MultiRunnable, Runnable, VERSION};
use crate::graph::{ArgKind, Graph, NodeId, ScatterOp};
